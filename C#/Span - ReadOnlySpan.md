...
`Span<T>` یک نوع Generic است که می‌تواند روی هر نوع Value Type یا Reference Type کار کند.
...
`ReadOnlySpan<T>` برای مواقعی استفاده می‌شود که می‌خواهیم فقط بخوانیم و نمی‌خواهیم داده را تغییر دهیم.
...
**پاسخ**:
- **Span**: می‌تواند تغییر دهد
- **ReadOnlySpan**: فقط Read-Only است
- هر دو Performance یکسانی دارند
- ReadOnlySpan برای داده‌های Immutable مناسب است

#### 4. محدودیت‌های Span چیست؟

**پاسخ**:
- **Stack-Only**: نمی‌تواند Field باشد
- **Contiguous Memory**: فقط برای Contiguous Memory کار می‌کند
- **Async**: نمی‌تواند در Async Methods استفاده شود
- **Heap**: نمی‌تواند در Heap ذخیره شود
- **Ref Struct**: یک Ref Struct است و محدودیت‌های خاص خود را دارد
